...
function makeRichTextLabelValue(label: string, value: string, colorView: boolean = true) {
  return [
    {
      text: `${label}: `,
      font: { bold: true, color: { argb: colorView ? 'ffffff' : 'ff000000' } },
    },
    { text: value, font: { bold: false, color: { argb: colorView ? 'ffffff' : 'ff000000' } } },
  ];
}

export function textLabelValueCell(label: string, value: string, colorView: boolean = true) {
  return {
...
interface CustomRow<T> {
  key: string;
  render: (data: T, colorView?: boolean) => string | { richText: any };
}

export interface CellDefinitionType<T> {
  [key: string]: {
    header: string;
    key: string;
    width: number;
...
const CELL_DEFINITION: CellDefinitionType<any> = {
  customer: {
    header: 'Customer',
    key: 'name',
    width: 20,
    ...
    rows: [
      {
        ...
        render: (client: ClientType, colorView) => {
          const salesRep = `${
            client.seller
              ? `${client.seller.name || ''} ${client.seller.last_name || ''}`.trim()
              : ''
          }`;
      ...
      },
      {
        ...
        render: (client: ClientType, colorView) => {
          const bdcRep = `${
            client.bdc ? `${client.bdc.name || ''} ${client.bdc.last_name || ''}`.trim() : ''
          }`;
      ...
      },
      {
        ...
        render: (client: ClientType, colorView) => {
          ...
          const assignedToSalesManagerRichText = makeRichTextLabelValue(
            'Sales Manager',
            salesManager,
            colorView,
          );
        ...
        render: (client: ClientType, colorView) => {
          const financeManager = `${
            client.finance_manager
              ? `${client.finance_manager.name || ''} ${
                  client.finance_manager.last_name || ''
                }`.trim()
              : ''
          }`;
          const assignedToFinanceManagerRichText = makeRichTextLabelValue(
            'Finance Rep',
            financeManager,
            colorView,
          );
    ...
    rows: [
      {
        key: 'created_date',
        render: (client: ClientType, colorView) =>
          textLabelValueCell('Created', formatDate(client.created_at), colorView),
      },
      {
...
export async function exportTableToExcel(
  data: any[],
  fileName = 'report-list.xlsx',
  colorView: boolean,
  options: options = { returnBlob: false, visibleColumns: [] },
  CELL_DEFINITION: CellDefinitionType<any>,
) {
