import { AppliedFilter, FilterableField, FilterableFieldType, FilterCondition, FilterValue } from "@/store/customerList/types";

const filterableFields: FilterableField[] = [
  { id: 'name_lastname', label: 'Customer Name', type: 'text' },
  { id: 'email', label: 'Email', type: 'text' },
  { id: 'client_address.city', label: 'City', type: 'text' },
  { id: 'client_address.state.state', label: 'State', type: 'text' },
  { id: 'client_address.zip', label: 'Zip Code', type: 'text' },
  { id: 'created_at', label: 'Created At', type: 'date' },
  { id: 'born_date', label: 'Date of birth', type: 'date' },
  { id: 'home_phone', label: 'Home Phone', type: 'text' },
  { id: 'work_phone', label: 'Work Phone', type: 'text' },
  { id: 'mobile_phone', label: 'Mobile Phone', type: 'text' },
  { id: 'interested_vehicle.vehicle_type.type', label: 'Vehicle Type', type: 'text' },
  { id: 'interested_vehicle.vehicle_models.model', label: 'Vehicle Model', type: 'text' },
  { id: 'interested_vehicle.vehicle_brands.brand', label: 'Vehicle Brand', type: 'text' },
...
export const AdvancedFilterRow: React.FC<AdvancedFilterRowProps> = ({ filter, onUpdate, onRemove }) => {
  ...
  const handleConditionChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    onUpdate({ ...filter, condition: e.target.value as FilterCondition, value: null, value2: null });
  };
  ...
  const renderValueInput = () => {
    ...
    switch (selectedFieldConfig.type) {
      ...
      case 'select':
        return (
          <select value={(filter.value as string) || ''} onChange={handleValueChange} className={commonInputClass}>
    ...
    <div className="text-sm flex gap-2 items-end p-3 border-b border-gray-200 w-full">
      <div className="md:col-span-1">
        ...
        >
          ...
          {filterableFields.sort((a, b) => a.label.localeCompare(b.label)).map(f => (
