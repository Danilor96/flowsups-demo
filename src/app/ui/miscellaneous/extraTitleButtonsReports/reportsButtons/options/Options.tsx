...
import { postCustomerReportAsFavorite } from '&/dashboard/clientSystem/customerList/CustomerReportsSelect/customerReport.service';
import { useSession } from 'next-auth/react';

export function Options({ toggleAddReportModal }: { toggleAddReportModal: () => void }) {
  // ----- global states -----
  ...
  const toggleNewCustomerReportModal = customerListStore(
    (state) => state.toggleNewCustomerReportModal,
  );
  const openCloseReportModal = customerListStore((state) => state.toggleSendReportModal);
  const [openClosepermissionsModal] = customerListStore((state) => [
  ...
  const handleSetAsFavorite = async () => {
    ...
    const currentIsFavorite =
      currentCustomerReport.favoriteBy && currentCustomerReport.favoriteBy.length > 0;
    ...
    try {
      const response = await postCustomerReportAsFavorite(
        currentCustomerReport.id,
        !currentIsFavorite,
      );
