...
import { OneFire, ThreeFires, TwoFires } from '@/app/ui/icons/Icons';
...
export function LeadTemperature() {
  ...
  const { openCloseBulkTemperature } = modalWindowStore();

  const { sellersData, clients, selectedCustomersIds, leadTemperatures } = adminDashboardStore();
  const { getSellers, getLeadTemperatures } = adminDashboardStore();
  ...
  const getPromiesesData = useCallback(() => {
    return [getSellers(), getLeadTemperatures()];
  ...
  const temperatureIconAndName = (id?: number) => {
    if (id === 1) {
      return (
        <span className="flex flex-row gap-[0.5vw] justify-center items-center px-auto">
          <p>Normal</p>
          <OneFire />
        </span>
      );
    ...
    if (selectedCustomersIds.length > 0) {
      ...
      for (let i = 0; i < selectedCustomersIds.length; i++) {
        ...
        newData.push({
          ...
          temperature: temperatureIconAndName(customer?.client_lead_temperature?.id),
        });
      }
  ...
  const handleClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (identity === 'temperature') {
      ...
      const statusSelected = leadTemperatures?.find(
        (temperature) => temperature.id.toString() === id,
      );

      setTemperature({
        id: id,
        name: `${statusSelected?.temperature || ''}`,
      });
    }
    ...
    if (identity === 'save') {
      ...
      formData.append('temperature', temperature.id);

      const apiUrl = `/api/bulkActions/leadTemperature`;

      await makeAsyncFetch({
        formData,
        apiUrl,
        method: 'POST',
        permissionForFetch: 62,
        options: {
          onSuccess: () => {
    ...
    <ModalWindow top={0} positionFixed>
      <ModalContainer width={85} marginTop={2.5}>
        <ModalContainerTitle
          ...
          closeWindowFunction={openCloseBulkTemperature}
        />
        <ModalContent>
          <ColoredTableV2
            data={tableData}
          ...
          />
          <ButtonContainer marginTop={2} widthFull justify="space-between" alignContentCenter>
            ...
            <AdderSelect
              ...
              options={leadTemperatures?.map((el) => {
                return {
