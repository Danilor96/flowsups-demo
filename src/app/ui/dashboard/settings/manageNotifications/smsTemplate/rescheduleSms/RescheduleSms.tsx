...
export function RescheduleSms() {
  ...
  const { openCloseRescheduleSms } = modalWindowStore();

  const { rescheduleSmsTemplate } = appointmentSmsTemplateStore();
  const { getRescheduleSmsTemplate } = appointmentSmsTemplateStore();
  ...
  const getPromisesData = useCallback(() => {
    return [getRescheduleSmsTemplate(), getSmsTemplateVariables()];
  }, []);
  ...
  useEffect(() => {
    if (rescheduleSmsTemplate) {
      setSms(rescheduleSmsTemplate.sms);
    }
  ...
  const handleSave = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const apiUrl = `/api/rescheduleSms/${rescheduleSmsTemplate?.id}`;
  ...
  return (
    ...
    >
      <ModalContainer marginTop={18} width={50}>
        <ModalContainerTitle
          title="Reschedule Sms Template"
          closeWindowFunction={openCloseRescheduleSms}
        />
        <ModalContent loading={loading || loadingFetch} minHeight={55}>
