...
import { getData } from './sales.services';
...
export function SalesReport({ closeFn }: { closeFn: () => void }) {
  // global states
  ...
  const [data, setData] = useState<ComissionSalesRepSummary[]>([]);
  ...
  const columnRenderers: { [key: string]: (el: ComissionSalesRepSummary) => any } = {
    salesConsultant: (el) => el.salesConsultant,
    sales: (el) => <button onClick={() => setUserId(el.salesConsultantId)}>{el.sales}</button>,
    commission: (el) => numberFormatter(el.comission, undefined, 1),
    spiff: (el) => (
      <button
        onClick={() => {
          setUserId(el.salesConsultantId);
          setType('spiff');
        }}
      >
  ...
  const { columns } = useDynamicTableColumns<ComissionSalesRepSummary, typeof initialColumnsDef>({
    columnRenderers,
    initialColumnsDef,
    accessorFnMapper: {
      salesConsultant: (el) => el.salesConsultant,
