...
import { AddressData, CreditAppData } from '@/app/api/adminDashboard/creditApp/types';
...
export interface DefaultAddress {
  client_address: {
    id: number;
    current_data_from_webhook: boolean;
    city: string;
    street: string;
    state_id: number;
    zip: string | null;
...
export function Address({
  ...
  const { numberFilter, numberFormatter } = numberFormatterStore();
  ...
  useEffect(() => {
    ...
    if (creditApp.address?.currentAddress) {
      ...
      street = defaultAddress?.client_address?.street || '';
      city = defaultAddress?.client_address?.city || '';
      state = defaultAddress?.client_address?.state?.state || '';
      zip = defaultAddress?.client_address?.zip || '';
      county = defaultAddress?.client_address?.county?.county || '';
    }
    ...
    setInputs({
      ...
      currentStateId:
        ...
        defaultAddress?.client_address?.state?.id.toString() ||
        '',
      currentZip: zip,
      ...
      newPreviousFormArray.push({
        ...
        prevInputs: [
          ...
          },
          {
            ...
            type: 'select',
            options: creditAppMonths?.map((el) => {
          ...
          },
          {
            ...
            type: 'select',
            options: creditAppAddressTypes?.map((el) => {
  ...
  useEffect(() => {
    ...
    if (creditAppDefault.address?.currentAddress) {
      const [currentStreet, currentCity, currentState, currentZip, currentCounty] =
        creditAppDefault.address.currentAddress.split(',');
    ...
    if (creditAppDefault.address?.mailingAddress) {
      const [
        currentMailingStreet,
        currentMailingCity,
        currentMailingState,
        currentMailingZip,
        currentMailingCounty,
      ] = creditAppDefault.address.mailingAddress.split(',');
    ...
    setInputs({
      id: creditAppDefault.address?.id?.toString() || '',
      currentAddress:
        creditAppDefault.address?.currentAddress ||
        `${street}, ${city}, ${state}${zip ? `, ${zip}` : ''}${county ? `, ${county}` : ''}` ||
        '',
      currentYear: creditAppDefault.address?.currentYear || '0',
      currentMonthId: creditAppDefault.address?.currentMonth?.toString() || '',
      currentAddressTypeId: creditAppDefault.address?.currentAddressType?.toString() || '',
      currentRentMort: creditAppDefault.address?.currentRentMortAmt || '',
      currentStreet: street,
      currentCity: city,
      currentState: state,
      currentStateId:
        creditAppDefault.address?.currentStateId?.toString() ||
        defaultAddress?.client_address?.state?.id.toString() ||
        '',
      currentZip: zip,
      currentCounty: county,
      mailingAddress: creditAppDefault.address?.mailingAddress || '',
      mailingStreet: mailingStreet,
      mailingCity: mailingCity,
      mailingState: mailingState,
      mailingStateId: creditAppDefault.address?.mailingStateId?.toString() || '',
      mailingZip: mailingZip,
      mailingCounty: mailingCounty,
      sameAsCurrent: creditAppDefault.address?.sameAsCurrentAddress ? '1' : '',
    });

    const newPreviousFormArray: PreviousAddressData[] = [];

    creditAppDefault.address?.prevAddress?.forEach((el) => {
      newPreviousFormArray.push({
        id: Number(el.id),
        prevAddress: el.address || '',
        prevStreet: '',
  ...
  const handleChangeCurrentAddress = (
    ...
    setInputs((prevState) => {
      ...
      switch (name) {
        ...
        case 'currentStateId':
          newState.currentState = states?.find((el) => el.id === parseInt(value))?.state || '';
          newState.currentStateId = value;

          if (newState.sameAsCurrent) {
            newState.mailingState = states?.find((el) => el.id === parseInt(value))?.state || '';
    ...
    for (let i = 0; i < previousAddressInputs.length; i++) {
      ...
      for (let elIndex = 0; elIndex < form.prevInputs.length; elIndex++) {
        ...
        if (formInput.name === 'month') {
          const monthSelected = creditAppMonths?.find(
  ...
  const handleNextPage = () => {
    ...
    const currentMonthsSelected = creditAppMonths
  ...
  const saveData = async () => {
    ...
    inputEditedRef.current = setTimeout(async () => {
      ...
      const apiUrl = `/api/public/creditApp/address/${customerId}`;
  ...
  useEffect(() => {
    setCurrentProgress(inputs, noCountInputs);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [inputs]);
  ...
  return (
    <aside className="relative md:mt-[2rem] lg:w-[70vw] lg:mx-auto">
      <ConfirmNotification
        notiMessage={confirmationMessage || confirmDeletePrevForm}
      ...
      <article className="px-[0.5rem] py-[0.5rem] flex flex-col gap-[1rem] md:grid md:grid-cols-3 md:gap-8 md:items-end">
        ...
        {inputData.map((el, index) => (
          <Input
            key={`${el.id * 935}000creditappcode${index + 1}`}
            label={el.label}
            name={el.name}
            type={el.type}
            width={el.width}
        ...
        <AddressInput
          ...
          manualStates={states}
          disabled={!!inputs.sameAsCurrent || loadingFetch}
        />
        <aside className="mb-auto">
          <Input
            ...
            chekcboxText="Mailing same as Current Address"
            customCheckbox
            isLoading={loadingFetch}
            disabled={loadingFetch}
            onChange={handleChange}
        ...
        {previousAddressInputs?.map((el, index) => (
          <aside
            key={`prevaddresspubliccredit${index}`}
            className="relative flex flex-col gap-2 py-[0.3rem] px-[0.15rem] md:col-span-3 lg:col-auto lg:w-full md:shadow-none md:px-[8rem] rounded-md shadow-crmFormShadow lg:px-0"
          >
            <AddressInput
              width={0}
              mainInput={{
            ...
            {showDeleteButtons && (
              <button
                ...
                data-identity="deletePreviousForm"
                value={index}
      ...
      <article className="flex flex-row justify-center items-center gap-[0.5rem] py-[0.2rem] mt-[1rem] bg-white">
        ...
        <button
          ...
          data-identity="addMorePreviousAddressForm"
          onClick={handleButton}
        >
          <PlusIcon mobile />
        </button>
        <button
          ...
          data-identity="showDeleteButtons"
          onClick={handleButton}
        >
          <TrashIcon mobile />
        </button>
      ...
      <article className="mt-[2vh] w-full px-[0.5rem] lg:px-[6vw] pb-[0.5rem] md:w-[50vw] md:mx-auto md:mt-[2.5vh] lg:w-[35vw]">
        <Button
          ...
          buttonTextSize={2}
          onClick={() => {
            if (nextToEmploymentStatus) {
              setCurrentPage(2);
            }
          }}
          disabled={!nextToEmploymentStatus || loadingFetch}
        />
      </article>
