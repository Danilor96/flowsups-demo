...
import { CarNotiIcon, MetricNotiIcon, ThreeDotsNotiIcon, TrashNotiDeleteIcon } from '&/icons/Icons';
...
export function Inventory({
  ...
  const handleNotiOption = async (
    ...
    if (e.currentTarget instanceof HTMLInputElement) {
      ...
      try {
        // Optimistic update
        if (notiId) {
             if (read && checked) optimisticMarkAsRead(notiId, true, 3); // 3 = Inventory
             if (unread && checked) optimisticMarkAsRead(notiId, false, 3);
        }
        ...
        if (notiId) {
             ...
             if (read && checked) optimisticMarkAsRead(notiId, false, 3); 
             if (unread && checked) optimisticMarkAsRead(notiId, true, 3);
        }
      }
    }
  ...
  const handleUserDecision = async (decision: boolean) => {
    if (decision) {
      ...
      try {
        // Optimistic update
        if (notiId) deletedItem = optimisticDelete(notiId, 3); 
        ...
        if (!res.successMessage) throw new Error('Failed to delete');
         ...
         // Precise Rollback
         if (deletedItem) restoreNotification(deletedItem, 3);
      }
    } else {
      setShowConfirmation(false);
    }
