...
export async function GET(
  ...
  try {
    ...
    const data = mockDb.deal.findFirst({
      where: {
        customer_id: customerId,
        ...dealWithLeadWhereClause,
      },
    });

    if (data) {
      const paymentDates = mockDb.paymentDate.findMany({
        where: { dealId: data.id },
      });

      const enrichedPaymentDates = paymentDates.map((pd: any) => ({
        ...pd,
        amountPerDate: mockDb.amountPerDate.findMany({
          where: { paymentDateId: pd.id },
        }),
      }));

      (data as any).paymentDate = enrichedPaymentDates;
      (data as any).bank = data.bank_id
        ? mockDb.banks.findUnique({ where: { id: data.bank_id } })
        : null;
    }
    ...
    const leadActive = mockDb.leads.findFirst({ where: leadWhereClause });

    const dealLeadActive = leadActive
      ? {
          id: leadActive.id,
          sold_created_at: leadActive.sold_created_at ?? null,
        }
      : null;

    revalidatePath(`${process.env.NEXTAUTH_URL}/dashboard`);

    return NextResponse.json({ deal: data, dealLeadActive });
  } catch (error) {
    console.log(error);
