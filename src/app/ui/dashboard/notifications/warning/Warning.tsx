...
export function Warning({
  ...
  const handleNotiOption = async (
    ...
    if (e.currentTarget instanceof HTMLInputElement) {
      ...
      try {
        // Optimistic update
        if (notiId) {
             if (read && checked) optimisticMarkAsRead(notiId, true, 5); // 5 = Warning
             if (unread && checked) optimisticMarkAsRead(notiId, false, 5);
        }
        ...
        if (notiId) {
             ...
             if (read && checked) optimisticMarkAsRead(notiId, false, 5); 
             if (unread && checked) optimisticMarkAsRead(notiId, true, 5);
        }
      }
    }
  ...
  const handleUserDecision = async (decision: boolean) => {
    if (decision) {
      ...
      try {
        // Optimistic update
        if (notiId) deletedItem = optimisticDelete(notiId, 5); 
        ...
        if (!res.successMessage) throw new Error('Failed to delete');
         ...
         // Precise Rollback
         if (deletedItem) restoreNotification(deletedItem, 5);
      }
    } else {
      setShowConfirmation(false);
    }
