...
import { ComissionBdcSummary } from '../types';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const comissionSummaryMap = new Map<number, ComissionBdcSummary>();
    const temporalComission = new Map<number, string>();

    users.forEach((user) => {
      if (!comissionSummaryMap.has(user.id)) {
        const bdcName = user.name;
        const bdcLastname = user.last_name;
        const bdcUsername = user.username;
        const bdcId = user.id;
    ...
    leads.forEach((lead) => {
      if (lead.bdc_id) {
        const mappedData = comissionSummaryMap.get(lead.bdc_id);
        ...
        if (mappedData) {
          ...
          const temporalMappedComission = temporalComission.get(lead.bdc_id) || '0';
