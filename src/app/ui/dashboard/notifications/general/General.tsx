...
import { MetricNotiIcon, ThreeDotsNotiIcon, TrashNotiDeleteIcon } from '&/icons/Icons';
...
export function General({
  ...
  const handleNotiOption = async (
    ...
    if (e.currentTarget instanceof HTMLInputElement) {
      ...
      try {
        // Optimistic update
        if (notiId) {
             if (read && checked) optimisticMarkAsRead(notiId, true, 1); // 1 = General
             if (unread && checked) optimisticMarkAsRead(notiId, false, 1);
        }
        ...
        if (notiId) {
             ...
             if (read && checked) optimisticMarkAsRead(notiId, false, 1); 
             if (unread && checked) optimisticMarkAsRead(notiId, true, 1);
        }
      }
    }
  ...
  const handleUserDecision = async (decision: boolean) => {
    if (decision) {
      ...
      try {
        // Optimistic update
        if (notiId) deletedItem = optimisticDelete(notiId, 1); 
        ...
        if (!res.successMessage) throw new Error('Failed to delete');
         ...
         // Precise Rollback
         if (deletedItem) restoreNotification(deletedItem, 1);
      }
    } else {
      setShowConfirmation(false);
    }
