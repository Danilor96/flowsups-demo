...
export function Sold() {
  ...
  const handleDealDateClick = (e: Date) => {
    setInputs((prevState) => ({
      ...prevState,
      dealDate: formatIncomingObjectDate(e),
    }));
  };
  ...
  const handleButton = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (identity === 'save') {
      try {
        ...
        const res = await fetch(`/api/adminDashboard/lead/sold/${singleCLientData?.id}`, {
          method: 'POST',
          body: formData,
        });
  ...
  return (
    <BorderedContent overflowVisible positionRelative loading={loading}>
      ...
      <ContentRow cols={2} gap={0} marginTop={3.148148} widthFull justifyContent="space-between">
        <Input
          label="Reminder Time"
          name="reminderTime"
        ...
        <Input
          ...
          type="DottedDate"
          width={15}
          value={inputs.dealDate}
          inputDate
          onChange={handleChange}
          onDayPickerClick={handleDealDateClick}
          fieldErrors={fieldErrors}
        />
      </ContentRow>
