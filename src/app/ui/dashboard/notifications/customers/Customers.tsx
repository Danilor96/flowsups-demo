...
import { ThreeDotsNotiIcon, TrashNotiDeleteIcon } from '&/icons/Icons';
...
export function Customers({
  ...
  // global states
  const { optimisticMarkAsRead, optimisticDelete, restoreNotification, getSingleClientTasks } =
    adminDashboardStore();

  const { showNotiOptions, openTaskDetail } = modalWindowStore();
  ...
  const handleNotiOption = async (
    ...
    if (e.currentTarget instanceof HTMLInputElement) {
      ...
      try {
        // Optimistic update
        if (notiId) {
          if (read && checked) optimisticMarkAsRead(notiId, true, 4); // 4 = Customers
          if (unread && checked) optimisticMarkAsRead(notiId, false, 4);
        }
        ...
        if (notiId) {
          ...
          if (read && checked) optimisticMarkAsRead(notiId, false, 4);
          if (unread && checked) optimisticMarkAsRead(notiId, true, 4);
        }
      }
    }
  ...
  const handleUserDecision = async (decision: boolean) => {
    if (decision) {
      ...
      try {
        // Optimistic update
        if (notiId) deletedItem = optimisticDelete(notiId, 4);
        ...
        // Precise Rollback
        if (deletedItem) restoreNotification(deletedItem, 4);
      }
    } else {
      setShowConfirmation(false);
    }
  ...
  const handleMessageLength = (message: string | undefined) => {
    if (message) {
      const mssgArray = message.split(' ');

      const returnedMssg =
        mssgArray.length > 4
          ? `${mssgArray[0]} ${mssgArray[1]} ${mssgArray[2]} ${mssgArray[3]} ...`
          : message;
    ...
    <>
      <section className="relative w-full flex flex-row items-center gap-[1.5vw] mt-[2.037037vh]">
        ...
        <div
          ...
          className={`flex flex-row items-center gap-[1.5vw] w-full ${taskId ? 'cursor-pointer' : ''}`}
        >
          <img
            src={`${
              img
          ...
          <div className="w-full flex flex-col gap-[1.574074vh]">
            <p className="text-[1.666667vh] font-normal text-[#676363]">
              <span className="font-bold">@{customer}</span> {message}
            </p>
              ...
              {!isRead && (
                <aside className="w-[11.197917vw] mx-auto flex flex-row gap-[0.653646vw] max-lg:w-full max-lg:px-2">
                  <input
                    ...
                    checked={false} // Ensure unchecked when re-rendering
                  />
